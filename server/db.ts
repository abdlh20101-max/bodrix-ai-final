...
export { users, messages, images, points, subscriptions, watchedAds, referrals, shares, wallets, walletTransactions, bankSettings, dailyChallenges, userChallengeProgress, siteConfig };
...
export async function updateSiteConfig(key: string, value: string, type: "string" | "number" | "boolean" | "json" = "string") {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
