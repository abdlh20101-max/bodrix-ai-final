...
import { LayoutDashboard, LogOut, PanelLeft, Users, Shield, MessageSquare, Wallet, Settings, Bell, Home } from "lucide-react";
...
const DEFAULT_MENU_ITEMS = [
  ...
  { id: "settings", label: "الإعدادات", path: "/settings", icon: "Settings", active: true },
];
  ...
  useEffect(() => {
    if (configs) {
      ...
      if (menuConfig && menuConfig.value) {
        try {
          const parsed = JSON.parse(menuConfig.value);
          if (Array.isArray(parsed)) {
            setMenuItems(parsed.filter((item: any) => item.active));
          }
        } catch (e) {
              ...
              {user?.role === "admin" && (
                <SidebarMenuItem>
                  <SidebarMenuButton
                    ...
                    className={`h-10 transition-all font-normal text-amber-500 hover:text-amber-600 hover:bg-amber-50/10`}
                  >
